            ...
            case R.id.getcord:
                if (!TextUtils.isEmpty(phone.getText().toString().trim())) {
                    if (phone.getText().toString().trim().length() == 11) {
                        ...
                        Toast.makeText(SMSActivity.this, "请输入完整电话号码", Toast.LENGTH_LONG).show();
                        phone.requestFocus();
                    }
                } else {
            ...
            case R.id.savecord:
                if (!TextUtils.isEmpty(cord.getText().toString().trim())) {
                    if (cord.getText().toString().trim().length() == 4) {
                        ...
                        Toast.makeText(SMSActivity.this, "请输入完整验证码", Toast.LENGTH_LONG).show();
                        cord.requestFocus();
                    }
                } else {
                    Toast.makeText(SMSActivity.this, "请输入验证码", Toast.LENGTH_LONG).show();
                    cord.requestFocus();
                }
                break;
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (result == SMSSDK.RESULT_COMPLETE) {
                ...
                if (flag) {
                    getCord.setVisibility(View.VISIBLE);
                    Toast.makeText(SMSActivity.this, "验证码获取失败，请重新获取", Toast.LENGTH_SHORT).show();
                    phone.requestFocus();
                } else {
                    ((Throwable) data).printStackTrace();
                    int resId = getStringRes(SMSActivity.this, "smssdk_network_error");
                    Toast.makeText(SMSActivity.this, "验证码错误", Toast.LENGTH_SHORT).show();
                    cord.selectAll();
                    if (resId > 0) {
                        Toast.makeText(SMSActivity.this, resId, Toast.LENGTH_SHORT).show();
                    }
                }
